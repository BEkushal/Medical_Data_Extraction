...
def doc_1_kathy():
    ...
    Migraine
    """
    return PatientDetailsParser(doc_txt)

@pytest.fixture()
def doc_2_jerry():
    doc_txt = document_text = '''
    Patient Medical Record
...
def test_parse(doc_1_kathy, doc_2_jerry):
    ...
    assert record_kathy['patient name'] == 'Kathy Crawford'
    assert record_kathy['phone number'] == '(737) 988-0851'
    assert record_kathy['medical issues'] == 'Migraine'
    assert record_kathy['hepatitis b vaccination'] == 'No'
    
    record_jerry = doc_2_jerry.parse()
    assert record_jerry == {
        'patient name': 'Jerry Lucas',
